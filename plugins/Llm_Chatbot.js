 ...
 * @text Add Character Memory
 * @desc Add new character memory to the memories array.
 * 
 * @arg memoryIdArg
 * @text Memory Id
 * @type string
 ...
 * ChatGPT_APIMZ for the fetch part. The plugin is using Koboldcpp to generate 
 * answer's text.
 * For a better explanation of each command and function, the plugin's 
 * documentation is written on my GitHub page, you can read it here:
 ...
 * ============================================================================
 * 
 * 1) You can change the url from the "text generaion url" plugin parameter.
 *
 ...
 * 
 * SYSTEM
 * Instructions for the event. It will be added to the SystemMessage of the plugin parameter, so
 * give this event any instructions you want to follow it with.
 * For example, if the parameter is set to "Answer in English",
 ...
 * 
 * CUSTOM MESSAGE
 *  Dai una domanda a cui l'Npc deve rispondere e un contesto per dirgli chi è e 
 *  come deve rispondere. Ogni volta che si interagisce con lui, risponderà a 
 *  quella domanda fissata. Viene generato un messaggio di testo, quindi il gioco 
 *  si interrompe finchè non lo leggi/clicchi/fai sparire il messaggio. Se 
 *  richiesto può mantenere in memoria dei dialoghi, quindi le sue risposte 
 *  terranno conto anche di essi. Ricorda però che più dialoghi si conservano, più 
 *  tempo è impiegato per generare la risposta.
 * 
 * GAB MESSAGE
 *  Fai comparire dei messaggi determinati su schermo senza interrompere il gioco.
 *  Puoi impostare la faccia del personaggio, la durata dopo cui il messaggio non 
 *  invasivo sparisce, e altre opzioni (colore, corsivo, opacità, etc).
 * 
 * AI GAB MESSAGE
 *  Come custom message, quindi quando l'Npc risponde non viene generata la classica
 *  finestra di messaggio, e il gioco non si interrompe. Puoi impostare la faccia 
 *  del personaggio, la durata dopo cui il messaggio non invasivo sparisce, e altre 
 *  opzioni (colore, corsivo, opacità, etc).
 * 
 * START DIALOGUE
 *  Devi impostare solo il contesto del personaggio. Sulla schermata di gioco apparirà 
 *  una finestra in cui puoi inserire il messaggio, usando la tastiera. Una volta che 
 *  il personaggio risponde, puoi chiedergli altre cose, e come per gli altri comandi 
 *  egli può ricordare i messaggi precedenti (se lo si desidera).
 * 
 * START WINDOWS SERVER
 *  Avvia Koboldcpp per Windows. Si apriranno delle finestre in cui puoi inserire le 
 *  opzioni più ottimali per te.
 * 
 * START MAC SERVER
 *  Avvia Koboldcpp per Mac. Si apriranno delle finestre in cui puoi inserire le 
 *  opzioni più ottimali per te. Mac e Linux hanno bisogno però di installare Koboldcpp, 
 *  ci possono volere intorno ai 10 minuti. (Il creatore di Koboldcpp ha creato l'exe 
 *  solo per windows.)
 * 【 ! Nota! 】
 *  Finchè non riesco ad avviare Start_Macos.sh dal plugin, bisogna avviarlo a mano (quindi 
 *  è inutile scegliere le opzioni desiderate dalle finestre che compaiono in-game). Trovi 
 *  Start_Macos.sh in chatbot folder. Devi anche aprire server.py con un editor di testo, e 
 *  mettere le opzioni desiderate.
 * 
 * START LINUX/WSL SERVER
 *  Bisogna avviarlo a mano. Trovi Start_Linux.sh/Start_Wsl.bat in chatbot folder. Devi 
 *  anche aprire server.py con un editor di testo, e mettere le opzioni desiderate.
 * 
 * EXAMPLE COLORS
 *  https://github.com/GiusTex/Llm-Chatbot/blob/main/docs/Examples.md#text-options
 * 
 * BLEND MODES
 * https://github.com/GiusTex/Llm-Chatbot/blob/main/docs/Examples.md#text-options
 * 
 * CUSTOM CHARACTERS
 * https://github.com/GiusTex/Llm-Chatbot/blob/main/docs/Examples.md#custom-characters
 */
...
(() => {
    ...
    PluginManager.registerCommand(pluginName, "gabMessage", async (args) => {
        ...
        allTextOptions.forEach(textOptionsObj => {
            ...
            universalOptions.currentTextHeight = textOptions.TextHeight;
            ...
            currentWordWrapWidth = textOptions.wordWrapWidth;
            //universalOptions...
        ...
        async function showMessage() {
            ...
            if (currentUseFace === "true" && currentTextX < 100) {
                currentTextX += 100;
            }
            ...
            const style = new PIXI.TextStyle({
                ...
                wordWrap: true,
                wordWrapWidth: currentWordWrapWidth
            });
    ...
    PluginManager.registerCommand(pluginName, "AiGabMessage", async (args) => {
        ...
        async function generateText() {
            await (async () => {
                ...
                try {
                    ...
                    //currentMessage = messageQueue.shift(currentTextMessage);
                    currentUseFace = messageQueue.shift(args.useFace);
                    currentUniversalOptions = messageQueue.shift(universalOptions);
                    ...
                    //let BetterCleanedText = "I don't mind if it's raining or not, rain is refreshing and makes everything look beautiful in the sky. Let's plan to meet up at the park near our building.";
                    ...
                    return new Promise((resolve) => {
                        const checkMenu = () => {
                            if (SceneManager._scene instanceof Scene_Menu) {
                                //console.log("Menu aperto, aspetto...");
        ...
        if (!generating) {
            messageQueue.push(args.useFace, universalOptions);
